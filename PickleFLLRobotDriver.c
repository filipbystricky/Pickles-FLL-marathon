...
void moveTestBetter(int power, int auxpower)  //Move the robot an arbitrary distance.  Encoder targets are supplied by the functions that call this one.
{
  reset();
  wait1Msec(50);
  //Initialize
  ...
  while ((!bothArrived(motors[motorRight].absdifference,motors[motorLeft].absdifference)) || !auxArrived(motors[motorAux].absdifference))
  {
    ...
    for (int cnt=0; cnt<3; cnt++)
    {
      ...
      if (cnt == 3) {
	      motors[cnt].power=calcBetterPower(abs(motors[cnt].absdifference), abs(motors[greatestValue].absdifference), abs(motors[leastValue].absdifference), abs(auxpower), motors[cnt].direction);
    	} else {
...
void driveAndDump(float distance, float aux, int distmotorpower, int auxmotorpower)
{
	...
	moveTestBetter(distmotorpower, auxmotorpower);
}
